...
### 4-3. ★ flagship 재-base (iter58): p* 비대칭점수-최적 점추정으로 메인 교체

멀티에이전트 종합심사 + 코드 직접 확인 결과, 구 1순위 ①(`per_bearing_robust.py`)은 9개 사전계산
벡터의 **메타-셀렉터**(`best_c=max(means)`)로, 발표 헤드라인("asym 직접최적화 argmax_p E[A]")과
**절차가 불일치**(narrative seam). 그 헤드라인을 *실제로* 구현하는 엔진 = `p_star_estimator.py`의
`p*=argmax_p E_neigh[asym(p,R)]` (train HI-KNN K=20). → **flagship을 p\*로 재-base.**

- 메인: `HUFS_validation_pstar.xlsx` = 29377/27574/65379/32394/1200/41400 (전 값 train 이웃 support 내).
- LOBO 0.5377(fold 정직공개: Train1 0.699 / Train2 0.543 / Train3 0.335 붕괴 / Train4 0.574).
- ① = 예비 Day2 **대조군**(mid-life short ~10k). 26(A) = 별도셋 전제 반증으로 대안 기록용.
- mid-life LONG은 HI-transfer **베팅**, T6=41400은 severity(짧음)와 충돌 → 6/1~5 예비 1비트 실측 판정.
- 상세·심사기준 매핑·caveat = `artifacts/submissions/BEST_METHOD_SELECTION.md`.
