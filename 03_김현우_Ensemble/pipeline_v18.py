"""v18: 진짜 챌린지 지표(rul_s, last-only) 최적화 + 4채널 피처 + EoL 강조

v17의 치명적 문제 (last-only 0.12) 해결.

핵심 변경:
  1. **rul_s 직접 학습** (rul_pw는 보조 task로만)
  2. **4채널 진동 피처** (ch0 → ch0~3 + cross-channel)
  3. **End-of-life 가중치 극대화**: 마지막 5측정 50x, 다음 10측정 10x
  4. **Asymmetric loss 강화**: 늦은 예측 5x 페널티 (2.5 → 5)
  5. **Multi-task loss**: rul_s (main) + rul_pw (aux) + EoL 분류 (aux)
  6. **Score loss 가중 증가**: alpha 0.7→0.6, score_loss 30% → 40%
  7. **Min RUL prior**: 모든 예측에 600s 하한 (true min = 600)
"""
import os
...
MODEL_DIR = METHOD_DIR / "models_v18"; MODEL_DIR.mkdir(parents=True, exist_ok=True)
SEQ_LEN = 10
...
print("=" * 70); print("  v18: rul_s 직접 + 4채널 + EoL 50x + asym 5x"); print("=" * 70)
...
def extract(s4, rpm, torque, tf, tr):
    ...
    # 채널 간 상관 (ch0-ch1, ch0-ch2, ch0-ch3, ch1-ch2)
...
for val in TRAIN_NAMES:
    ...
    plt.tight_layout(); plt.savefig(RESULT_DIR / f"v18_{val}.png", dpi=120); plt.close()

res = pd.DataFrame(results)
print("\n" + "=" * 70, flush=True); print("  v18 최종 결과 (rul_s + 4채널 + EoL 50x)", flush=True); print("=" * 70, flush=True)
...
res.to_csv(RESULT_DIR / "lobo_v18.csv", index=False)
