...
def extract_v20(s4, rpm, torque, tf, tr):
    feats = {}
    per_ch = {}
    for ci in range(4):
...
# 3-way 블렌딩 함수
def blend_3way(p17, p18, p20, hi, thr1, thr2, slope, beta=1.0):
    """3구간 sigmoid 블렌딩."""
    ...
    # w20 = 1 - w17 - w18 (clip ≥ 0)
    w20 = np.clip(1.0 - w17 - w18, 0, 1)
    # normalize
    s = w17 + w18 + w20 + 1e-12
    w17, w18, w20 = w17/s, w18/s, w20/s
    p = w17 * p17 + w18 * p18 + w20 * p20
...
def main():
    ...
    preds20, hi20 = get_predictions(MODEL_DIRS["v20"], extract_v20, DTCVAE_v18, TRAIN_NAMES)

    oof = {}
    for val in TRAIN_NAMES:
        p17 = preds17[val][val]; p18 = preds18[val][val]; p20 = preds20[val][val]
        p17m = np.median(np.clip(p17, 0, None), 0)
        p18m = np.median(np.clip(p18, 600, None), 0)
        p20m = np.median(np.clip(p20, 600, None), 0)
        # HI: v18+v20 평균 (가장 풍부한 피처 기반)
        hi = 0.5 * hi18[val] + 0.5 * hi20[val]
        L = min(len(p17m), len(p18m), len(p20m), len(hi))
        _, op = load_bearing(val)
        yvl = op["rul_seconds"].values[-L:]
        oof[val] = dict(p17=p17m[-L:], p18=p18m[-L:], p20=p20m[-L:],
                        hi=hi[-L:], yvl=yvl,
                        t_h=op["t_seconds"].values[-L:]/3600)
        print(f"  {val}: L={L}  p17[-1]={p17m[-1]:.0f}  p18[-1]={p18m[-1]:.0f}  p20[-1]={p20m[-1]:.0f}",
              flush=True)
        ...
        for thr2 in [0.65, 0.7, 0.75, 0.8, 0.85, 0.9]:
            ...
            for slope in [10, 20, 40, 60]:
                for beta in [0.90, 0.95, 1.00, 1.05, 1.10]:
                    ...
                    for val in TRAIN_NAMES:
                        o = oof[val]
                        p = blend_3way(o["p17"], o["p18"], o["p20"], o["hi"],
    ...
    for name, fn in [
        ...
        ("v20 only", lambda o: np.clip(o["p20"], 600, None)),
        ("simple avg 3-way", lambda o: np.clip((o["p17"]+o["p18"]+o["p20"])/3, 600, None)),
    ]:
    ...
    tpreds20, thi20 = get_predictions(MODEL_DIRS["v20"], extract_v20, DTCVAE_v18, VAL_NAMES, target_folds=TRAIN_NAMES)
    ...
    for nm in VAL_NAMES:
        ...
        p20m_folds = [np.median(np.clip(tpreds20[nm][f], 600, None), 0) for f in TRAIN_NAMES if f in tpreds20[nm]]
        p17m = np.median(np.array(p17m_folds), 0)
        p18m = np.median(np.array(p18m_folds), 0)
        p20m = np.median(np.array(p20m_folds), 0)
        hi = 0.5 * thi18[nm] + 0.5 * thi20[nm]
        L = min(len(p17m), len(p18m), len(p20m), len(hi))
        p17m=p17m[-L:]; p18m=p18m[-L:]; p20m=p20m[-L:]; hi=hi[-L:]
        p_full = blend_3way(p17m, p18m, p20m, hi,
                             best_full.thr1, best_full.thr2, best_full.slope, best_full.beta)
        p_comb = blend_3way(p17m, p18m, p20m, hi,
        ...
        rows_out.append({"Bearing": nm, "N_meas": len(op),
                         ...
                         "RUL_v20_s": float(p20m[-1]),
                         "RUL_blend_full_s": float(p_full[-1]),
                         "RUL_blend_combined_s": float(p_comb[-1])})
        print(f"  {nm}: HI={hi[-1]:.2f}  v17={p17m[-1]:.0f}  v18={p18m[-1]:.0f}  v20={p20m[-1]:.0f}  "
              f"blend_full={p_full[-1]:.0f}  blend_comb={p_comb[-1]:.0f}", flush=True)

    out_df = pd.DataFrame(rows_out)
    out_full = RESULT_DIR / "submission_v21_3way_full.xlsx"
    out_comb = RESULT_DIR / "submission_v21_3way_combined.xlsx"
    ...
    out_df.to_csv(RESULT_DIR / "submission_v21_debug.csv", index=False)
    print(f"\n  → {out_full}")
    print(f"  → {out_comb}")
