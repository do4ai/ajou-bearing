"""태환팀: VMD + STFT Spectrogram + 2D CNN-LSTM — 전체 파이프라인 (RUL in seconds)"""
import os
...
from shared.utils import asym_score, load_bearing, TRAIN_NAMES, FS
...
def vmd_approx(sig, K=4, fs=FS):
    ...
    for k in range(K):
        lo, hi = max(bounds[k], 1), min(bounds[k+1], nyq*0.99)
        b, a = butter(4, [lo/nyq, hi/nyq], btype="band"); modes.append(filtfilt(b, a, sig))
    return np.array(modes)

def spectrogram(sig, fs=FS, nfft=256, hop=128, fmax=5000):
    ...
    return zoom(S.astype(np.float32), (SPEC_H / S.shape[0], SPEC_W / S.shape[1]))
...
def load_specs(name):
    ...
    for i in range(min(5, len(sigs))):
        ...
        k_sel = int(np.argmax([sp_kurt(m) for m in modes])); base.append(spectrogram(modes[k_sel]))
...
class CNN2D(nn.Module):
    ...
    def forward(self, x):
        B, T, C, H, W = x.shape; e = self.enc(x.view(B*T, C, H, W)).view(B*T, -1)
        p = self.proj(e).view(B, T, 128); l, _ = self.lstm(p); w = self.attn(l); ctx = (l * w).sum(1)
...
for nm in TRAIN_NAMES:
    sp, op = load_specs(nm)
    all_specs[nm] = sp; all_ops[nm] = op; all_ops[nm]["rul_s"] = op["rul_seconds"]
...
for val in TRAIN_NAMES:
    ...
    def mk_seqs(nm, sl=SEQ_LEN):
        sp = all_specs[nm]; rul = all_ops[nm]["rul_s"].values
        s, t = [], []
    ...
    tr_s = np.concatenate([mk_seqs(n)[0] for n in tns]); tr_t = np.concatenate([mk_seqs(n)[1] for n in tns])
    vl_s, vl_t = mk_seqs(val)

    m = CNN2D(SPEC_H, SPEC_W); opt = optim.AdamW(m.parameters(), lr=LR, weight_decay=1e-4)
    sch = optim.lr_scheduler.CosineAnnealingLR(opt, T_max=EPOCHS)
    # RUL 정규화
    rul_max = max(tr_t.max(), 1.0); tr_tn = tr_t / rul_max; vl_tn = vl_t / rul_max
    tld = DataLoader(DS(tr_s, tr_tn), batch_size=8, shuffle=True); vld = DataLoader(DS(vl_s, vl_tn), batch_size=16)
    best_sc, best_st = -np.inf, None
    for ep in range(1, EPOCHS+1):
        m.train()
        for xb, yb in tld: opt.zero_grad(); aloss(m(xb), yb).backward(); nn.utils.clip_grad_norm_(m.parameters(), 1.); opt.step()
        sch.step(); m.eval()
        ...
        if ep % 20 == 0: print(f"    Ep{ep:3d}/{EPOCHS} Score={s:.4f} RMSE={np.sqrt(mean_squared_error(vl_t, vp)):.0f}s")

    m.load_state_dict(best_st); m.eval()
    with torch.no_grad(): preds = np.nan_to_num(np.concatenate([m(xb).numpy() for xb, _ in vld])) * rul_max
    rmse = np.sqrt(mean_squared_error(vl_t, preds)); score = asym_score(preds, vl_t)
    ...
    th = all_ops[val]["t_seconds"].values[SEQ_LEN:] / 3600
    fig, ax = plt.subplots(1, 2, figsize=(12, 4))
    ax[0].plot(th, vl_t / 3600, "k-", lw=2, label="True"); ax[0].plot(th, preds / 3600, "r--", label="Pred")
    ax[0].set(xlabel="Time(h)", ylabel="RUL(h)", title=f"{val} CNN-LSTM"); ax[0].legend()
    ax[1].imshow(all_specs[val][-1, 0], aspect="auto", origin="lower", cmap="viridis")
    ax[1].set(title=f"{val} Spectrogram", xlabel="Time frames", ylabel="Freq bins")
